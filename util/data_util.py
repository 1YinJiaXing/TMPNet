...
def collate_fn(batch):
    coord, label = list(zip(*batch))  # 解包 batch，只有 coord 和 label
    coord = [torch.tensor(c, dtype=torch.float32) for c in coord]  # 转换为 torch.Tensor
    label = [torch.tensor(l, dtype=torch.long) for l in label]  # 转换为 torch.Tensor
    ...
    return torch.cat(coord), torch.cat(label), torch.IntTensor(offset)
    #return torch.cat(coord), torch.cat(label)
