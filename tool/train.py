...
from util.lingjian import CustomDataset  # 从dataset.py导入CustomDataset


def get_parser():
    parser = argparse.ArgumentParser(description='PyTorch Point Cloud Semantic Segmentation')
    parser.add_argument('--config', type=str, default='/home/ubunone/YJX/fenge/point-transformer-master/config/Lingjian/Lingjian-mamba.yaml', help='config file')
...
def main():
    ...
    if args.data_name == 'lingjian':
        train_data = CustomDataset(split='train', root=args.data_root)
        val_data = CustomDataset(split='test', root=args.data_root)
    else:
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'TMPNet':
        from model.TMPNet import TMPNet as Model
    ...
    if main_process():
        ...
        logger.info(f"Total model parameters: {total_params:.2f}M")
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, label,offset) in enumerate(train_loader):  # (n, 3), (n)
        data_time.update(time.time() - end)
        coord, label,offset = coord.cuda(non_blocking=True), label.cuda(non_blocking=True), offset.cuda(non_blocking=True)
        #print(' coord ',coord.shape )
        #print(' label ', label.shape)
        # Prepare input in (p, x, o) format
        p = coord  # (n, 3)
        x = torch.zeros_like(coord)
        o = offset

        #data_dict = {"coord": coord, "feat": x, "offset": offset}
        data_dict = [p, x, o]
        output = model(data_dict)

       # output = model([p, x, o])
        if label.shape[-1] == 1:
            label = label[:, 0]  # for cls
        ...
        if args.multiprocessing_distributed:
            ...
            n = count.item()
            loss /= n
        intersection, union, target = intersectionAndUnionGPU(output, label, args.classes, args.ignore_label)
        if args.multiprocessing_distributed:
...
def validate(val_loader, model, criterion):
    ...
    for i, (coord, label,offset) in enumerate(val_loader):
        data_time.update(time.time() - end)
        coord, label,offset = coord.cuda(non_blocking=True), label.cuda(non_blocking=True), offset.cuda(non_blocking=True)

        # Prepare input in (p, x, o) format
        p = coord  # (n, 3)
        x = torch.zeros_like(coord)  # Assuming no additional features, use zeros
        o = offset
        ...
        if args.multiprocessing_distributed:
            loss *= n
            count = label.new_tensor([n], dtype=torch.long)
            dist.all_reduce(loss), dist.all.reduce(count)
            n = count.item()
            loss /= n

        intersection, union, target = intersectionAndUnionGPU(output, label, args.classes, args.ignore_label)
        if args.multiprocessing_distributed:
            dist.all.reduce(intersection), dist.all.reduce(union), dist.all.reduce(target)
        intersection, union, target = intersection.cpu().numpy(), union.cpu().numpy(), target.cpu().numpy()
        intersection_meter.update(intersection), union_meter.update(union), target_meter.update(target)
